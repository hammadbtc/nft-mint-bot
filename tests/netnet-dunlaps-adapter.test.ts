...
test("DUNLAPS is pinned to the Robinhood public SeaDrop phase only", () => {
  ...
  assert.equal(project.maxPerWallet, 2);
...
test("DUNLAPS public calldata has exact target, recipient, zero delegated minter, and quantity", () => {
  const data = encodeSeaDropPublicMint(project.contractAddress, config.feeRecipient, ethers.ZeroAddress, 2);
  ...
  assert.equal(BigInt(project.mintPrice) * decoded.quantity, 4_000_000_000_000_000n);
});
