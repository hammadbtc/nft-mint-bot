...
export default function DispersePage() {
  ...
  useEffect(() => {
    Promise.all([fetch("/api/wallets").then((response) => response.json()), fetch("/api/chains").then((response) => response.json())])
      .then(([walletData, chainData]) => {
        ...
        if (activeChains.length) setChainId(Number(activeChains[0].id));
      }).catch(() => undefined);
  }, []);
  ...
  return <><div className="page-heading"><div><h1>Disperse</h1><p>Fund or sweep the same wallet set on any supported EVM network.</p></div></div><div className="panel disperse-grid"><div className="form-grid"><div className="segmented"><button className={type === "fund" ? "active" : ""} onClick={()=>{setType("fund");setPreview(null)}}>Fund workers</button><button className={type === "sweep" ? "active" : ""} onClick={()=>{setType("sweep");setPreview(null)}}>Sweep to main</button></div><div className="field"><label>Network</label><select value={chainId} onChange={(event)=>{setChainId(Number(event.target.value));setPreview(null)}}>{chains.map((chain)=><option key={chain.id} value={chain.id}>{chain.name}</option>)}</select></div><div className="field"><label>Main wallet</label><select value={main} onChange={(event)=>{setMain(event.target.value);setSelected(new Set());setPreview(null)}}><option value="">Choose main wallet</option>{mains.map((wallet)=><option key={wallet.id} value={wallet.id}>{wallet.label}</option>)}</select></div><div className="field"><label>Workers ({selected.size} selected)</label><div className="wallet-picker">{workers.length ? workers.map((wallet)=><label className="wallet-option" key={wallet.id}><input type="checkbox" checked={selected.has(wallet.id)} onChange={()=>toggle(wallet.id)}/><span>{wallet.label}</span><small>{wallet.address.slice(0,7)}…{wallet.address.slice(-5)}</small></label>) : <div className="wallet-option muted">Choose a main wallet with workers</div>}</div></div>{type === "fund" && <div className="field"><label>Amount per worker</label><input value={amount} onChange={(event)=>{setAmount(event.target.value);setPreview(null)}} inputMode="decimal" placeholder="0.01"/></div>}</div><div className="form-grid"><div className="summary-box"><div className="summary-line"><span>Network</span><b>{chains.find((chain)=>chain.id === chainId)?.name || `Chain ${chainId}`}</b></div><div className="summary-line"><span>Direction</span><b>{type === "fund" ? "Main → workers" : "Workers → main"}</b></div><div className="summary-line"><span>Workers</span><b>{selected.size}</b></div><div className="summary-line"><span>Transfers</span><b>{preview?.transfers.length || 0}</b></div><div className="summary-line"><span>{type === "fund" ? "Total required" : "Estimated return"}</span><b>{preview ? ethers.formatEther(preview.totalRequiredWei) : "—"}</b></div><div className="summary-line"><span>Estimated network fee</span><b>{preview ? ethers.formatEther(preview.estimatedGasWei) : "—"}</b></div></div>{message && <div className="alert">{message}</div>}{!preview ? <button className="primary-btn" disabled={busy || !main || !selected.size || (type === "fund" && !amount)} onClick={requestPreview}>{busy ? "Checking balances & fees…" : "Review disperse →"}</button> : <><div className="alert">Review complete. The chain, amount and fee ceiling are locked for 60 seconds.</div><button className="primary-btn" disabled={busy} onClick={execute}>{busy ? "Submitting…" : type === "fund" ? "Confirm & fund workers" : "Confirm & sweep to main"}</button></>}</div></div></>;
}
