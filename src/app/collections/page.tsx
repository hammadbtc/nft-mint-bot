...
export default function CollectionsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowForm(false);
      setForm({ name: "", contractAddress: "", chainId: 1, mintMethod: "mint", mintAbi: "", mintPrice: "" });
      fetchData();
    } catch (err: any) {
        ...
        <form onSubmit={handleSubmit} className="mb-8 p-4 bg-zinc-900 rounded-xl border border-zinc-800 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                type="text"
                value={form.mintMethod}
                onChange={(e) => setForm({ ...form, mintMethod: e.target.value })}
                className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-white text-sm"
                placeholder="mint"
              />
            </div>
          ...
          <div>
            <label className="block text-sm text-zinc-400 mb-1">
              Mint ABI <span className="text-zinc-600">(JSON fragment for mint function)</span>
            </label>
            <textarea
              value={form.mintAbi}
              onChange={(e) => setForm({ ...form, mintAbi: e.target.value })}
              className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-white text-sm font-mono h-24"
              placeholder='[{"type":"function","name":"mint","inputs":[{"type":"uint256","name":"quantity"}],"outputs":[],"stateMutability":"payable"}]'
              required
            />
          </div>
          <button
              ...
              {collections.map((c) => (
                <tr key={c.id} className="border-b border-zinc-800/50">
                  ...
                  <td className="py-2 pr-4">
                    {chains.find((ch) => ch.id === c.chainId)?.name || `Chain ${c.chainId}`}
                  </td>
