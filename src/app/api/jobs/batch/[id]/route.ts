...
export async function DELETE(req: NextRequest, { params }: Context) {
  try {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      const mutableError = jobs.map((job) => mintTaskMutationError(job.status, false)).find(Boolean);
      if (mutableError) throw new Error(`Whole schedule was not deleted: ${mutableError}`);
      ...
      const deleted = await tx.delete(schema.mintJobs)
        .where(and(inArray(schema.mintJobs.id, jobs.map((job) => job.id)), eq(schema.mintJobs.status, "pending")))
