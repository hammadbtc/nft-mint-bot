      ...
      <section className="panel mint-card">
        ...
        <div className="mint-body"><div className="mint-grid"><div className="phase-list">
          {(project.phases?.length ? project.phases : [{ id: "phase", name: project.phaseName || "Mint phase", status: project.phaseStatus, startsAt: project.startsAt || undefined, endsAt: project.endsAt || undefined, priceWei: project.mintPrice || undefined, maxPerWallet: project.maxPerWallet || undefined }]).map((phase) => {
            ...
            return <div className="phase" key={phase.id}><div className="phase-top"><h3>{phase.name.toUpperCase()}</h3><span className="status">{phase.status === "live" ? "Live" : phase.status === "upcoming" ? "Upcoming" : phase.status === "ended" ? "Ended" : "Unknown"}</span></div><div className="chip-row"><span className="chip">PRICE · <strong>{formatPrice(phase.priceWei || null)}</strong></span><span className="chip">MAX · <strong>{phase.maxPerWallet || "—"}</strong></span><span className="chip">ELIGIBLE · <strong>{checkingEligibility ? "…" : `${eligibleCount}/${compatible.length}`}</strong></span>{unknownCount > 0 && <span className="chip">UNKNOWN · <strong>{unknownCount}</strong></span>}</div>{phase.startsAt && <div className="muted" style={{fontSize:12,marginTop:9}}>{phase.status === "upcoming" ? `Opens ${formatContractTime(phase.startsAt)}` : `Started ${formatContractTime(phase.startsAt)}`}</div>}{eligibilityReason && <div className="muted" style={{fontSize:12,marginTop:7}}>{eligibilityReason}</div>}</div>;
          })}
